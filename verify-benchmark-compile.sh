...
run_build() {
    ...
    if [ -n "$SYSROOT" ]; then
        # Cross-compilation: execute natively inside the sysroot using chroot.
        sudo chroot "$SYSROOT" /bin/bash -c "cd $TEMP_DIR/workloads && $cmd" 2>&1 | tee "$log_file"
    else
        # Native compilation: run directly.
        /bin/bash -c "cd $TEMP_DIR/workloads && $cmd" 2>&1 | tee "$log_file"
    fi
